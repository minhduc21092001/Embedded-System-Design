...
extern EventGroupHandle_t computer_sending_eventgroup;
extern EventBits_t computer_sending_bit;
...
void threshold_handle(void)
{
	...
	{
		xEventGroupSetBits(computer_sending_eventgroup, BIT_0);
		HAL_GPIO_WritePin(GPIOA,GPIO_PIN_5,GPIO_PIN_SET);
	}
	else
	{
		xEventGroupClearBits(computer_sending_eventgroup, BIT_0);
		HAL_GPIO_WritePin(GPIOA,GPIO_PIN_5,GPIO_PIN_RESET);
	}
	if(index11.RH >= hum_threshold)
	{
		xEventGroupSetBits(computer_sending_eventgroup, BIT_1);
		HAL_GPIO_WritePin(GPIOA,GPIO_PIN_6,GPIO_PIN_SET);
	}
	else
	{
		xEventGroupClearBits(computer_sending_eventgroup, BIT_1);
		HAL_GPIO_WritePin(GPIOA,GPIO_PIN_6,GPIO_PIN_RESET);
	}
	if(adc_mq2_index>= gas_threshold)
	{
		xEventGroupSetBits(computer_sending_eventgroup, BIT_2);
		HAL_GPIO_WritePin(GPIOA,GPIO_PIN_7,GPIO_PIN_SET);
	}
	else
	{
		xEventGroupClearBits(computer_sending_eventgroup, BIT_2);
